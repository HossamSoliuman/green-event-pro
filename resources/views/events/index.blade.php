    ...
    @if ($events->isEmpty())
        <div class="card text-center py-16">
            ...
            <p class="text-gray-500 text-sm mb-6">
                {{ __('Create your first sustainable event and start with the UZ 62 certification.') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            @foreach ($events as $event)
                <div class="card hover:shadow-md transition-shadow group">
                    ...
                    <a href="{{ route('events.show', $event) }}">
                        <h3 class="font-semibold text-gray-900 hover:text-green-600 transition-colors mb-1">
                            {{ $event->title }}</h3>
                    </a>
                    ...
                    <div class="flex items-center gap-4 text-xs text-gray-500 mb-4">
                        ...
                        <span class="flex items-center gap-1">
                            ...
                            {{ number_format($event->expected_participants) }} {{ __('Participants') }}
                        </span>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        @if ($event->carbon_footprint_per_person)
                            <div class="flex justify-between text-xs">
                                ...
                                <span
                                    class="{{ $event->carbon_footprint_per_person <= 30 ? 'text-green-600' : ($event->carbon_footprint_per_person <= 100 ? 'text-yellow-600' : 'text-red-600') }} font-medium">
