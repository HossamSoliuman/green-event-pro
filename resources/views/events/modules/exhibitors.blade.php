@extends('layouts.app')
@section('title', ucfirst('exhibitors').' – '.$event->title)
@section('page-title', 'Modul: '.ucfirst(str_replace('_',' ','exhibitors')))
@section('content')
<div class="mb-4">
    <a href="{{ route('events.show', $event) }}" class="text-sm text-green-600 hover:text-green-700">← Zurück zur Veranstaltung</a>
</div>
<form method="POST" action="{{ route('events.modules.exhibitors.update', $event) }}">
    @csrf @method('PUT')
    <div class="card space-y-4">
        <h3 class="section-title">{{ ucfirst(str_replace('_',' ','exhibitors')) }} Daten</h3>
