@extends('layouts.app')
@section('title', ucfirst('communication').' – '.$event->title)
@section('page-title', 'Modul: '.ucfirst(str_replace('_',' ','communication')))
@section('content')
<div class="mb-4">
    <a href="{{ route('events.show', $event) }}" class="text-sm text-green-600 hover:text-green-700">← Zurück zur Veranstaltung</a>
</div>
<form method="POST" action="{{ route('events.modules.communication.update', $event) }}">
    @csrf @method('PUT')
    <div class="card space-y-4">
        <h3 class="section-title">{{ ucfirst(str_replace('_',' ','communication')) }} Daten</h3>
        ...
        <div>
            ...
            <label class="flex items-center gap-2">
                <input type="checkbox" name="{{ $field }}" value="1" {{ $val ? 'checked' : '' }} class="form-checkbox">
                <span class="text-sm text-gray-600">Ja</span>
            </label>
            @elseif(is_numeric($val) || is_null($val))
            <input type="number" step="0.01" name="{{ $field }}" value="{{ $val }}" class="form-input">
            @else
