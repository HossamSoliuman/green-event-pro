    ...
    <div class="card">
        <form method="POST" action="{{ route('events.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <div class="mt-4">
                    <label class="form-label">{{ __('Expected Participants') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="venue_country" class="form-select">
                                <option value="AT" {{ old('venue_country', 'AT') === 'AT' ? 'selected' : '' }}>{{ __('Austria') }}</option>
                                <option value="DE" {{ old('venue_country') === 'DE' ? 'selected' : '' }}>{{ __('Germany') }}</option>
                                <option value="CH" {{ old('venue_country') === 'CH' ? 'selected' : '' }}>{{ __('Switzerland') }}</option>
            ...
            <div>
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="text-sm text-gray-700">{{ __('Outdoor / Open-Air Event (Vb criteria apply)') }}</span>
                    </label>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_hybrid" value="1" {{ old('is_hybrid') ? 'checked' : '' }} class="form-checkbox">
                        <span class="text-sm text-gray-700">{{ __('Hybrid Event (M17 becomes MUST criteria)') }}</span>
                    </label>
                </div>
            </div>
