...
<div class="flex min-h-screen">
    ...
    <aside class="hidden lg:flex flex-col w-64 bg-white border-r border-gray-200 fixed inset-y-0 z-50">
        ...
        <div class="px-4 py-3 border-b border-gray-100">
            ...
            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-50 text-green-700 mt-1">
                {{ ucfirst($currentOrganization->subscription_plan ?? __('Free')) }}
            </span>
        </div>
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <div class="pt-4 pb-1">
                <p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Organization') }}</p>
            </div>
    ...
    <aside x-show="sidebarOpen" class="lg:hidden fixed inset-y-0 left-0 w-64 bg-white z-50 overflow-y-auto">
        ...
        <div class="flex items-center gap-2 px-6 py-5 border-b border-gray-100">
            ...
            <span class="font-bold text-gray-900 text-sm">GreenEventPro</span>
            <button @click="sidebarOpen = false" class="ml-auto text-gray-400 hover:text-gray-600">
        ...
        <nav class="px-3 py-4 space-y-1">
            ...
            <a href="{{ route('events.index') }}" @click="sidebarOpen = false" class="sidebar-link {{ request()->routeIs('events.*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                {{ __('Events') }}
            </a>
            <a href="{{ route('analytics') }}" @click="sidebarOpen = false" class="sidebar-link {{ request()->routeIs('analytics*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                {{ __('Analytics') }}
            </a>
            ...
            <a href="{{ route('organization.settings') }}" @click="sidebarOpen = false" class="sidebar-link {{ request()->routeIs('organization.settings') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                {{ __('Settings') }}
            </a>
            <a href="{{ route('organization.users') }}" @click="sidebarOpen = false" class="sidebar-link {{ request()->routeIs('organization.users*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                {{ __('Users') }}
            </a>
        </nav>
    ...
    <div class="flex-1 lg:pl-64 flex flex-col min-h-screen">
        ...
        <header class="bg-white border-b border-gray-200 px-4 lg:px-8 py-4 flex items-center justify-between sticky top-0 z-40 min-h-16 flex-wrap gap-2 sm:flex-nowrap">
            ...
            <div class="w-full order-4 sm:hidden flex-shrink-0">
                @yield('header-actions')
            </div>
        </header>
        ...
        <footer class="px-8 py-4 text-center text-xs text-gray-400 border-t border-gray-100">
            GreenEventPro &copy; {{ date('Y') }} — {{ __('Sustainable events according to UZ 62') }}
        </footer>
    </div>
</div>
